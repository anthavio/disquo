package com.nature.jersey.server;
 ...
 */
public class JsonSchemaWadlGenerator implements WadlGenerator {
	...
	private static final Logger LOGGER = Logger.getLogger(JsonSchemaWadlGenerator.class.getName());

	private static final java.util.Set<Class<?>> SPECIAL_GENERIC_TYPES = new HashSet<Class<?>>() {
		{
			add(JResponse.class);
			add(List.class);
		}
	...
	public JsonSchemaWadlGenerator() {
	...
	public Param createParam(AbstractResource ar, AbstractMethod am, Parameter p) {
		...
		if (p.getSource() == Parameter.Source.ENTITY) {
			_hasTypeWantsName.add(new Pair(parameter(p), new WantsName() {
				public void setName(QName name) {
					param.setType(name);
				}
			}));
		...
		if (responses != null) {
			...
			for (Response response : responses) {
				for (final Representation representation : response.getRepresentation()) {
					...
					_hasTypeWantsName.add(new Pair(hasType, new WantsName() {
						@Override
						public void setName(QName name) {
	...
	public ExternalGrammarDefinition createExternalGrammar() {
		...
		Map<String, ApplicationDescription.ExternalGrammar> extraFiles = new HashMap<String, ApplicationDescription.ExternalGrammar>();
	...
	private Resolver buildModelAndSchemas(Map<String, ExternalGrammar> extraFiles) {
		...
		try {
			...
			for (StreamResult result : results) {
				...
				extraFiles.put(result.getSystemId(), new ApplicationDescription.ExternalGrammar(MediaType.APPLICATION_XML_TYPE, // I don't think there is a specific media type for XML Schema
						contents));
			}
